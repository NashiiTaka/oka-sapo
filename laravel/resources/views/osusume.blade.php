        ...
        @foreach ($recommendations as $recommendation)
          <div class="min-w-[270px] bg-white p-4 border rounded-xl shadow flex flex-col items-center gap-y-2">
            ...
            <div class="h-28 flex items-center justify-center">
              <img class="max-h-28" src="{{ $recommendation->getImagePath() }}" alt="{{ $recommendation->valiations[0]->valiation_name }}">
            </div>
            <p class="text-xs font-semibold">{{ $recommendation->maker->maker_name }} : {{ $recommendation->brand->brand_name }}</p>
              ...
              <div class="overflow-x-auto">
                <div class="flex space-x-4">
                  @foreach ($recommendation->valiations as $valiation)
                    <div class="min-w-10 min-h-10 rounded-full" style="background-color: {{ $valiation->hex_color_code }}">
                    </div>
                  @endforeach
                </div>
              </div>
            ...
            <div class="flex flex-row gap-x-4">
              ...
              <x-a-button href="{{ $recommendation->buy_url }}" display="購入" target="_blank" px="6" py="2" />
              <x-a-button href="/face-detection/{{ $recommendation->product_id }}" display="試着" target="_blank" px="6" py="2" />
              @endif
            </div>
          </div>
        @endforeach
